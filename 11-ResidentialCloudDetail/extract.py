...
def boxplot(data, pos, cm, outliers_lower=5, outliers_upper=10, forget=False):
    ...
    return (
        f"\\addplot+ [boxplot prepared={{lower whisker={lw:.2f}, lower quartile={lq:.2f}, median={me:.2f}, upper quartile={uq:.2f}, upper whisker={uw:.2f}, }}, "
        f"boxplot/draw position={pos}, index of colormap={cm}, {'forget plot, ' if forget else ''} mark=+, every mark/.append style={{mark size=1pt, line width=0.2pt}}] table\n"
...
if __name__ == "__main__":
    ...
    colors = [0, 0, 0, 1, 1, 1, 4, 4, 3]
    position, coloridx = 1, 0

    for ep in ["aws", "aks"]:
        for id in range(1, 10):

            data = parse_e2e_file(args.input_path, f"{id:02d}-{ep}-*.csv")
            text = boxplot(data, position, colors[coloridx], outliers_lower=10, outliers_upper=30,
                forget=(coloridx > 0 and colors[coloridx] == colors[coloridx-1]))
            print(text)

            position = position + 1
            coloridx = (coloridx + 1) % 9
